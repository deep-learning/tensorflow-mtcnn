...
void nms_boxes(std::vector<face_box> &input, float threshold, int type, std::vector<face_box> &output) {
    std::sort(input.begin(), input.end(), [](const face_box &a, const face_box &b) {
...
void square_boxes(std::vector<face_box> &rects) {
    for (auto &rect : rects) {
        float h = rect.y1 - rect.y0 + 1;
        float w = rect.x1 - rect.x0 + 1;

        float l = std::max(h, w);

        rect.x0 = rect.x0 + (w - l) * 0.5F;
        rect.y0 = rect.y0 + (h - l) * 0.5F;
        rect.x1 = rect.x0 + l - 1;
        rect.y1 = rect.y0 + l - 1;
    }
}
...
void padding(int img_h, int img_w, std::vector<face_box> &rects) {
    for (auto &rect : rects) {
        rect.px0 = std::max(rect.x0, 1.0f);
        rect.py0 = std::max(rect.y0, 1.0f);
        rect.px1 = std::min(rect.x1, (float) img_w);
        rect.py1 = std::min(rect.y1, (float) img_h);
    }
}
